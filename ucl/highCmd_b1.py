...
class highCmd:
    ...
    def buildCmd(self, debug=False):
        ...
        cmd[65:69] = float_to_hex(self.dComXy[0])
        cmd[69:73] = float_to_hex(self.dComXy[1])
        cmd[73:77] = float_to_hex(self.dstandFootXy[0])
        cmd[77:81] = float_to_hex(self.dstandFootXy[1])
        cmd[81:85] = self.bms.getBytes()
        cmd[85:89] = self.led.getBytes()
        cmd[89:129] = self.wirelessRemote
        cmd[129:133] = self.reserve
        if self.encrypt:
